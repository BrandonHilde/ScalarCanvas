...
function handleDrop(e) {
  ...
  infoLeft.textContent = `Loading ${file.name}...`;
  infoRight.textContent = '';
  glyphsEl.innerHTML = '';
  const reader = new FileReader();
  reader.onload = ev => {
    try {
      const parser = new TTFParser(ev.target.result);
      parser.parse();
      render(parser, file.name);
    } catch (err) {
...
function render(parser, filename) {
  const chars = [];
  for (let i = 32; i < 127; i++) {
    const ch = String.fromCharCode(i);
    const idx = parser.charToGlyphIndex(ch);
    if (idx !== undefined && idx !== 0) {
      const g = parser.getGlyph(idx);
      if (g) chars.push({ char: ch, ...g });
    }
  }

  infoLeft.textContent = filename;
  infoRight.textContent = `${chars.length} glyphs rendered`;
  glyphsEl.innerHTML = '';

  const pad = Math.max(40, parser.unitsPerEm * 0.04);

  chars.forEach(({ char, path, xMin, yMin, xMax, yMax }, index) => {
    const w = (xMax - xMin) + pad * 2;
    const h = (yMax - yMin) + pad * 2;

    const card = document.createElement('div');
    card.className = 'glyph-card';
    card.style.animationDelay = `${Math.min(index * 20, 600)}ms`;
    ...
    card.innerHTML = `
      <div class="glyph-preview">
        <svg viewBox="0 0 ${w} ${h}" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid meet">
          <g transform="translate(${pad - xMin}, ${pad}) scale(1, -1) translate(0, -${yMax})">
            <path d="${path}" />
          </g>
        </svg>
      </div>
